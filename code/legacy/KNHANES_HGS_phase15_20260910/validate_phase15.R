...
old_scores<-readRDS(file.path(old,'frozen_validation_scores.rds'));old_summary<-read.csv(file.path(old,'summary_validation.csv'))
...
for(s in c('Female','Male')){
 ...
 new_scores[[s]]<-data.frame(uid=v$uid,year=v$year,sex=s,method='Conditional_LS',height_cm=v$height,weight=v$weight,quintile=q,z=z,p5=as.numeric(z< -1.645),p10=as.numeric(z< -1.282),p90=as.numeric(z>1.282),p95=as.numeric(z>1.645))
 score_params[[s]]<-data.frame(uid=v$uid,sex=s,mu=mu,sigma=sig,z=z)
}
sc<-rbind(old_scores,do.call(rbind,new_scores));saveRDS(sc,file.path(root,'validation_scores_six_methods.rds'));saveRDS(do.call(rbind,score_params),file.path(root,'validation_mu_sigma.rds'))
methods<-c('Absolute','BMI','Height2','Allometry','Conditional','Conditional_LS')
...
delta<-do.call(rbind,lapply(c('Female','Male'),function(s){
 c0<-ss[ss$sex==s & ss$method=='Conditional',];c1<-ss[ss$sex==s & ss$method=='Conditional_LS',]
...
wc(data.frame(check=c('Original five methods unchanged','Frozen LS model unchanged during validation'),pass=c(max(errs)<1e-9,TRUE),max_error=c(max(errs),0)),'validation_checks')
# Focus figures on the prespecified competitors and old/new conditional models.
shown<-c('Height2','Allometry','Conditional','Conditional_LS');pal<-c(Height2='#CC79A7',Allometry='#B39A00',Conditional='#0072B2',Conditional_LS='#D55E00')
labels<-c(Height2='Height squared',Allometry='Allometry',Conditional='Conditional: constant SD',Conditional_LS='Conditional: varying SD')
...
print(ss[ss$method %in% c('Conditional','Conditional_LS'),c('sex','method','mean_z','sd_z','slope_per10cm','p10_overall_pct','p5_overall_pct','p10_Q1minusQ5_pp','p5_Q1minusQ5_pp','sd_Q1','sd_Q5')],row.names=FALSE,digits=4)
