...
sc<-readRDS(file.path(root,'validation_scores_six_methods.rds'));params<-readRDS(file.path(root,'validation_mu_sigma.rds'))
...
for(s in c('Female','Male')){
 ...
 a<-sc[sc$sex==s & sc$method=='Conditional_LS',];a<-a[match(v$uid,a$uid),]
 ...
 w<-d$weight/mean(d$weight);p<-ncol(Xd);theta<-c(b$mu_coefficients,b$log_sigma_coefficients)
 fn<-function(t){mu<-as.numeric(Xd%*%t[1:p]);ls<-as.numeric(Xd%*%t[p+1:p]);sum(w*(ls+.5*(d$hgs-mu)^2*exp(-2*ls)))}
 ...
 r<-ss[ss$sex==s & ss$method=='Conditional_LS',];slope<-sum(a$weight*(a$height_cm/10-weighted.mean(a$height_cm/10,a$weight))*(a$z-weighted.mean(a$z,a$weight)))/sum(a$weight*(a$height_cm/10-weighted.mean(a$height_cm/10,a$weight))^2)
 add(paste(s,'independent slope'),abs(slope-r$slope_per10cm)<1e-10,abs(slope-r$slope_per10cm))
 for(q in paste0('Q',1:5)){
  aa<-a[a$quintile==q,];rr<-qq[qq$sex==s & qq$method=='Conditional_LS' & qq$quintile==q,]
